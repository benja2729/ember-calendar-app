
define [
  'ValpoUtils/Core'
  'ValpoUtils/ArrayPromise', 'ValpoUtils/ObjectPromise'
  'ValpoUtils/ModalView', 'ValpoUtils/StatefulRoute'
  'ValpoUtils/Promisible', 'ValpoUtils/Stateful', 'ValpoUtils/StatefulDate'
  'ValpoUtils/closestBinarySearch', 'ValpoUtils/DateProxy'
  'ValpoUtils/Tile'

  'ValpoUtils/ButtonView', 'ValpoUtils/BackButtonView', 'ValpoUtils/SidePaneView',
  'ValpoUtils/BitObject'
], (VU) ->

   window.ValpoUtils = window.VU = VU