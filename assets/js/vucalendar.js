...
App.iCalMixin = Em.Mixin.create({
  ...
  formatDate: function(event, key) {
    var date, format, isAllDay, range, _ref;
    format = this.get('format');
    _ref = event.getProperties('range', 'isAllDay'), range = _ref.range, isAllDay = _ref.isAllDay;
...
utcTime = function(key) {
  return Em.computed('range', function() {
    return this.formatDate(this, key);
  });
};
...
App.ApplicationController = Em.Controller.extend(App.DataUtilMixin, {
  ...
  currentController: Em.computed('currentResource', function() {
    return this.controllerFor(this.get('currentResource'));
  }),
  iCalEventsCache: Em.computed('currentController.content', 'filters.categories', function() {
    var content, controller, result;
    controller = this.get('currentController');
    result = controller.get('filteredEvents');
    result = result === void 0 && App.Event.detectInstance(content = controller.get('content')) ? Em.A([content]) : result;
    console.log(Em.Array.detect(result) ? result.get('length') : null);
    return result || Em.A();
  }),
